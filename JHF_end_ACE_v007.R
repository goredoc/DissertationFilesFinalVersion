#JHF_end_ACE_v007
# changed to LBA where alpha=beta=1 all trials
# keep shape and rate parameters from last time
# fix A at 1
# Run for 2000 iterations
# Fixed updating rules (LBA)
# 

#JHF_end_ACE_v006
# should use GORE updating rule
# fix A at 1
# keep the shape and rate parameters from last time
# Run for 2000 iterations
# Goal is to get BALBA fits using GORE updating
# Handled Source, RData, PNG
# Fixed updating rules (GORE)

#JHF_end_ACE_v003 as above but fixes rho at 0 and makes iota zero
# this creates a non sequential version of BALBA
#JHF_end_ACE_v002 deploys these variances
# the shape and rate parameters on the variance are adjusted
# in this version. Very high rate drives the spread around
# gamma variances down 
# updating rule is alpha(t) = rho*alpha(t-1) + I <if last is RIGHT stim>

#JHF_end_ACE_v001 used to estimated variances emprically
# note that rho was set to zero causing problem in logit
# these should be re-done if needed 
# may not be needed because only used for variance

stopCluster(cl)
rm(list = ls())
set.seed(1)
library(beepr)
# Previously saved as recovery_hierarchical.R
...
source("JHF_end_priors_init_v007.R")
...
params_trans <- exp(params_true)
#for (s in 1:nrow(params_true)) {
#  params_trans[s, ] <- _to_balba_space_probit(params_true[s, ])[-c(8)]
#  }
params_trans
...
sourceCpp("BALBA.cpp")
source("JHF_end_BALBA2_v007.R")
source("JHF_end_priors_reduced_v007.R")
...
# num_monte_carlo creates a stop point of ### iterations

# raise the limit to increase current to best
...
for (i in 1:num_chains) {
  ...
  out <- foreach(s = 1:S, .combine = cbind, .packages = c("Rcpp"), 
    ...
    while (current_weight == -Inf) {
      ...
      debugonce(log_dens_like_jones)
      current_weight <- log_dens_like_jones (params = current_thetas, 
                                      data = Data[[s]], 
                                      auxdata = DataExtras[[s]],
                                      pnum = s,
...
for (i in (I - 1):num_monte_carlo) {
  ...
  cat("\n ", i, "  ")
  if (i %% 100 == 0) save.image("JHF_end_v007_centsave.RData")
  # Hyper level
  phi[,, i] <- phi[,, i - 1]
...
out_array <- foreach(s = 1:S, 
                     .combine = bind_3, # combine on third dimension look into it
                     .packages = c("Rcpp"), 
                     .noexport = c("logdens_balba")) %dopar% {
    sourceCpp("BALBA.cpp")
    # debugonce(crossover_jones)
    out <- sapply(1:num_chains,
                  crossover_jones,
  ...
  # put just the likelihood in second index? 
...
I <- i
toc <- tictoc::toc()
save.image("JHF_end_v007_A.RData")
### return to line 211 or so when socklist error
theta[, , 1, 25]
...
save.image("JHF_end_v007_B.RData")
...
#pairs.panels(t(phi_scatter))
png("phiScatter_JHF_end_ACE_v007.png",width=7.5, height=7.5, units="in",res=72)
pairs.panels(t(phi_scatter[c(1,3,5,7,9,11,13),]))
dev.off()
...
save.image("JHF_end_v007_C.RData")
#--------------------------- Prediction
# Synth_data <- list()
# s <- 15
