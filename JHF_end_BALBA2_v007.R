...
simulate_exp <- function(pnum,
  ...
  for(bl in 1:num_blocks){
    ...
    alpha[1] <- beta[1] <- 1
    for(i in 2:n){
      #alpha[i] <- rho * (alpha[i-1]-1) + 2^(2 - stim_vec[i-1])
      #beta[i]  <- rho * (beta[i-1]-1) + 2^(stim_vec[i-1] - 1)
      alpha[i] <- beta[i] <- 1
    }
...
log_dens_like <- function(params, 
  ...
  for(bl in 1:num_blocks){
    ...
    alpha[1] <- beta[1] <- 1
    for(i in 2:n){
      #alpha[i] <- params["rho"] * (alpha[i-1]-1) + 2^(2 - stim_vec[i-1])
      #beta[i]  <- params["rho"] * (beta[i-1]-1)  + 2^(stim_vec[i-1] - 1)
      alpha[i]=beta[i]=1
    }
    for(cond_i in 1:6){
