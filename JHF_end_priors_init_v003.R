...
param_names <- c("v1_A", "v1_B", "v1_C", "v2", "b_acc", "b_speed")#
param_names_fixed <- c("A","rho")
params_fixed <- c(A,rho)
...
start_points <- mu_mean_vec <- c(log(v1_A), log(v1_B), log(v1_C), log(v2), log(b_acc), log(b_speed))
start_sds <- mu_sd_vec <-      c(        1,         1,         1,       1,          1,            1)
sigma_shape_vec        <-      c(   .574e5,    .574e5,    .574e5,  .574e5,    2.692e5,      2.692e5) 
sigma_rate_vec         <- rep(1e5,length(param_names))
