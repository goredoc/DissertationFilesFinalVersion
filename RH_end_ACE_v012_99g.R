# purpose of RH_end_ACE_v012_99g.R
# use Gore updating rule
# set rho=.99
# fit 16 cases worth of sim data
# use 1500 iterations
# otherwise leave RH_end_ACE_v002.R unchanged
...
stopCluster(cl)
rm(list = ls())
# seed(1) faulted during generation of cases (several runs)
# seed(2) faulted with as parallelization related error
#         when initially starting to sample
#         Error in { : task 14 failed = "missing value where TRUE/FALSE needed"
set.seed(3)
...
sourceCpp("BALBA.cpp")
source("RH_end_BALBA2_v012_99g.R")
source("RH_end_priors_init_v012_99g.R")
...
sourceCpp("BALBA.cpp")
source("RH_end_BALBA2_v012_99g.R")
source("RH_end_priors_reduced_v012_99g.R")
source("RH_end_DEMCMC_v012_99g.R")
...
for (i in (I - 1):num_monte_carlo) {
  ...
  cat("\n ", i, "  ")
  if (i %% 100 == 0) save.image("RH_end_v012_99g_centsave.RData")
  # Hyper level
  phi[,, i] <- phi[,, i - 1]
...
save.image("RH_end_v012_99g_A.RData")
rho1a = phi[13,1,1:i]
...
save.image("RH_end_v012_99g_B.RData")
...
save.image("RH_end_v012_99g_C.RData")
#--------------------------- Prediction
# Synth_data <- list()
# s <- 15
