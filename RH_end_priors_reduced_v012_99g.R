...
start_sds <- mu_sd_vec <- rep(1,length(start_points))     #c(1, 1, 1, 1, 1, 1, 1)
sigma_shape_vec  <- rep(1,length(start_points))   #c( 1, 1, 1, 1, 1, 1, 1)
sigma_rate_vec   <- rep(1,length(start_points))   #c( 1, 1, 1, 1, 1, 1, 3) # 7.4 makes 3sd of p from .4 to .6
# 7.4 was value in fit001
# that was not showing good signs of heading toward true value (mean post burnin .55 not .45)
# and failed after about 1000 runs without being restartable (11-16-2020, 11 PM)
